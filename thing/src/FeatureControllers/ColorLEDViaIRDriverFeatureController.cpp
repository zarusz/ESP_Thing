...
ColorLEDViaIRDriverFeatureController::ColorLEDViaIRDriverFeatureController(
    int port, DeviceContext *context, int pin)
    : IRFeatureController(port, context, pin) {
}

void ColorLEDViaIRDriverFeatureController::Handle(const char *topic,
...
void ColorLEDViaIRDriverFeatureController::SendSignal(int value,
                                                      const char *command) {
  sprintf(_logger->Msg(),
          "[ColorLEDViaIRDriverFeatureController] Port %d: Sending command %s "
          "- IR: (format: NEC, bits: %d, data: %s)\n",
