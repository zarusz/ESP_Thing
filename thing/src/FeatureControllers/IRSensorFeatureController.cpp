...
IRSensorFeatureController::IRSensorFeatureController(int port,
                                                     DeviceContext *context,
                                                     int pin)
    : FeatureController(port, FeatureType::SENSOR_IR, context), _irrecv(pin) {
}
...
void IRSensorFeatureController::Loop() {
  if (_irrecv.decode(&_results)) {
    ...
    if (_results.decode_type == UNKNOWN) {
      Serial.printf(
          "[IRSensorFeatureController] Received IR: unknown, rawlen: %d\n",
          _results.rawlen);
    } else {
      Serial.printf("[IRSensorFeatureController] Received IR: port %d, format: "
                    "%s, bits: %d, data: %s\n",
                    _port, formatLabel, _results.bits,
                    String((int)_results.value, HEX).c_str());
...
void IRSensorFeatureController::Publish(decode_results &results,
                                        IRFormat format) {
  /*
