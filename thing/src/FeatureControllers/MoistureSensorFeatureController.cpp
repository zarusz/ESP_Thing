...
MoistureSensorFeatureController::MoistureSensorFeatureController(
    int port, DeviceContext *context, int pinSelect, int pinAdc)
    : FeatureController(port, FeatureType::SENSOR_IR, context) {
...
void MoistureSensorFeatureController::Loop() {
  ...
  if (_valuePublished == -1 ||
      abs(_valuePublished - _value) > _publishThreshold) {
