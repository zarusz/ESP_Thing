...
MqttMessageBus::MqttMessageBus(DeviceConfig *deviceConfig,
                               MessageHandler *handler)
    : _mqttClient(_espClient) {
  _deviceConfig = deviceConfig;
  SetHandler(handler);

  _mqttClient.setServer(_deviceConfig->MqttHost.c_str(),
                        _deviceConfig->MqttPort);
...
bool MqttMessageBus::Publish(const char *topic, const char *message,
                             bool retained) {
  Serial.printf(
      "[MQTT] Publish to topic: '%s', payload size: %d, payload: %s\n", topic,
...
void MqttMessageBus::ReconnectMqtt() {
  ...
  while (!_mqttClient.connected()) {
    ...
    String clientId =
        String(_deviceConfig->UniqueId) + "-" + String(random(0xffff), HEX);
    ...
    if (_willTopic && _willMessage) {
      connected =
          _mqttClient.connect(clientId.c_str(), _deviceConfig->MqttUser.c_str(),
                              _deviceConfig->MqttPass.c_str(), _willTopic,
                              MQTTQOS1, _willRetain, _willMessage);
    } else {
      connected =
          _mqttClient.connect(clientId.c_str(), _deviceConfig->MqttUser.c_str(),
                              _deviceConfig->MqttPass.c_str());
    }
...
void MqttMessageBus::MqttMessageCallback(char *topic, byte *payload,
                                         uint length) {
  if (Singleton)
...
void MqttMessageBus::SetWill(const char *topic, const char *message,
                             bool retain) {
  _willTopic = topic;
